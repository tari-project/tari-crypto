                ...
                .map_err(|e| RangeProofError::InvalidRangeProof { reason: e.to_string() })
            {
                ...
                Err(e) => {
                    return Err(RangeProofError::InvalidRangeProof {
                        reason: format!("Range proof at index '{}' could not be deserialized ({})", i, e),
                    });
                },
            }
        }
                    ...
                    for value in [0, 1, u64::MAX] {
                            ...
                            value >> (bit_length - 1) <= 1
                        {
                            ...
                            assert!(bulletproofs_plus_service.verify(
                                &proof.unwrap(),
                                factory.commit_value(&key, value).borrow(),
                                &mut rng
                            ));
                        } else {
                            assert!(proof.is_err());
        ...
        for extension_degree in [
            ...
            for bit_length in BIT_LENGTH {
                ...
                for aggregation_size in AGGREGATION_SIZE {
                    ...
                    // 4. Create the proof
                    let proof =
                        bulletproofs_plus_service.construct_extended_proof(extended_witnesses, seed_nonce, &mut rng);
